...
export async function handleChatMessage(options: ChatHandlerOptions): Promise<ChatHandlerResult> {
  if (!GEMINI_API_KEY) {
    return {
      success: false,
      thread_id: options.thread_id ?? 'unknown',
  ...
  if (options.mode === 'task') {
    try {
      ...
      return {
        success: false,
        thread_id: threadId,
        content: `Error en el agente colaborativo: ${(err as Error).message}`,
        tool_calls: [],
      }
    }
  }
  ...
  return {
    success: true,
    thread_id: threadId,
    content: 'El agente realizó varias consultas pero no generó una respuesta final. Intenta reformular tu pregunta.',
    reasoning: finalReasoning || undefined,
    tool_calls: toolCallsSummary,
  }
}
